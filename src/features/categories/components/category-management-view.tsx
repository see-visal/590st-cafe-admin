...
import type { Category, CategoryRequest, CategoryStatus } from "@/features/categories/types/category.type";
...
export default function Categories() {
  ...
  const apiPage = usePagination(filteredCategories);
  const mockPage = usePagination(STATIC_CATEGORY_ROWS);
  const activePage = hasApiData ? apiPage : mockPage;

  const handleExport = () =>
    downloadCsv("categories", filteredCategories as never[], autoColumns(filteredCategories as never[]));
    ...
    <PageShell>
      ...
      >
        <SimpleTable headers={[...CATEGORY_TABLE_HEADERS]}>
          {hasApiData
            ? apiPage.pageRows.map((category, index) => (
            ...
            : mockPage.pageRows.map((category, index) => (
                <Row key={category.id} striped={index % 2 === 1}>
                  <Cell>{index + 1}</Cell>
                  <Cell>
                    <Thumbnail />
        ...
        <ModalGrid>
          ...
          <FormSelect
            ...
            onValueChange={(e) =>
              setFormFields({
                ...formFields,
                status: (e || undefined) as CategoryStatus | undefined,
              })
            }
          >
            <SelectItem value="ACTIVE">Active</SelectItem>
