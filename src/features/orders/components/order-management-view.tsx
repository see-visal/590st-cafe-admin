...
export default function Orders() {
  ...
  const apiPage = usePagination(orders);
  const mockPage = usePagination(STATIC_ORDER_ROWS);
  const activePage = hasApiData ? apiPage : mockPage;

  const handleExport = () =>
    downloadCsv("orders", orders as never[], autoColumns(orders as never[]));
    ...
    <PageShell>
      ...
      >
        <SimpleTable headers={[...ORDER_TABLE_HEADERS]}>
          {hasApiData
            ? apiPage.pageRows.map((order, index) => (
            ...
            : mockPage.pageRows.map((order, index) => (
                <Row key={order.id} striped={index % 2 === 1}>
                  <Cell>{index + 1}</Cell>
                  <Cell>
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-gray-900 text-white shadow-xs">
